...
export class UpdateProductCommandMysqlImpl implements UpdateProductCommandHandler {
  ...
  async execute(command: UpdateProductCommand): Promise<Product | null> {
    ...
    try {
      ...
      await this.replaceProductDetails(connection, command.id, variants, sizes);
